...
| Automated web discovery and candidate staging | Implemented | Automated discovery via Brave Search crawls public sources into `corpus_staging` with status `pending_review`. Requires explicit Legal Corpus Reviewer promotion before entering authoritative corpus. Migration 20260907_0009. |
| Session document uploads and temporary retrieval | Implemented | Session-scoped document ingestion (`session_uploads`, `session_upload_chunks`) supporting PDF, DOCX, TXT, HTML, CSV with vector search. Quarantined under `USER_UPLOAD` jurisdiction with 24-hour auto-purge TTL. |
| Multilingual text delivery | Implemented | Bhashini query translation and language audit paths in backend; full frontend UI localization across English, Hindi, Kannada, and Tamil (`frontend/src/i18n/resources.ts`). |
| Full multilingual voice experience | Implemented | Backend Bhashini ASR/TTS endpoints integrated; browser speech synthesis with Indian accent voice matching and clean legal text synthesis. |
